...
export default async function deleteFolderCloudinary(
  pathToDelete: string
): Promise<boolean> {
  try {
    // **Step 1: Delete all files in the folder and subfolders**
    await cloudinary.api.delete_resources_by_prefix(pathToDelete);

    // **Step 2: Delete the empty folder (and all subfolders)**
    await cloudinary.api.delete_folder(pathToDelete);

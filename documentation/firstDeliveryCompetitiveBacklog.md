...
> **Purpose:** Extract from [`appCompetition/webapps.md`](appCompetition/webapps.md) every competitor capability relevant to Equus **first delivery** priorities: (1) **user + horse details for social interaction**, and (2) **stable SaaS**.  
...
- [`userModule.md`](userModule.md) — § First delivery market backlog
- [`horseModule.md`](horseModule.md) — § First delivery market backlog
- [`stableModule.md`](stableModule.md) — § First delivery market backlog
- [`mvpScope.md`](mvpScope.md) — Phase 1A / first delivery priority
- Full competitor detail: [`appCompetition/webapps.md`](appCompetition/webapps.md)
...
| **B — Stable SaaS** | Multi-user barn ops: roster, tasks, health, feed, docs, finance, facilities, owner transparency | Equicty, HippoVibe, Equestrian App (StallPros), EquineM parity already in `stableModule.md` |
...
Extracted from **Equicty (Equstable/Equclub)**, **HippoVibe**, **Equestrian App (StallPros)**, and cross-checked with existing [`stableModule.md`](stableModule.md) / EquineM in [`appCompetition/webapps.md`](appCompetition/webapps.md#12-equinem). Items already in `stableModule.md` are marked **already specified**; new market gaps are marked **add**.
...
> Riding club as a separate entity remains in `riding-clubs.md`; first delivery may ship **lesson-capable stable** subset only if product chooses.

### B.9 Breeding (pro stables / studs)

| Market feature | Competitor(s) | Status vs Equus |
|----------------|---------------|-----------------|
| Mare cards, cycles, AI, embryo, foaling | Equicty Breeding, EquineM | **Defer** to post-first-delivery (mvpScope / EquineM stud in `appCompetition/webapps.md` §12) unless ICP is stud |
...
| Date | Change |
|------|--------|
| 2026-07-24 | Initial extraction from `appCompetition/webapps.md` for first-delivery social (user/horse) + stable SaaS; linked into module specs |

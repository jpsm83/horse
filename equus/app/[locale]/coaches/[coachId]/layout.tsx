/**
 * Coach layout — RSC that pre-fetches the role-aware coach view once per
 * navigation.
 *
...
import { getCoachView } from "@/lib/services/coachService.ts";
...
export default async function CoachLayout({ children, params }: CoachLayoutProps) {
  ...
  try {
    ...
    if (!canRecoverSession) {
      const data = await getCoachView(coachId, userId);
      queryClient.setQueryData(queryKeys.coaches.view(coachId), data);
    }
  } catch {
  ...
  return (
    <PreferHydrationBoundary state={dehydrate(queryClient)}>
      <CoachLayoutChrome coachId={coachId}>{children}</CoachLayoutChrome>

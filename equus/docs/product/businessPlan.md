...
Related context:
- `equus/docs/engineering/stack.md` — canonical technical stack (Next.js, REST API, MongoDB, Auth.js, shadcn, Zod)
- `mvpScope.md` — build phases, production launch gate, and exclusions
- `equus/docs/features/stableModule.md` — stable feature specification (living doc; EquineM parity + more)
- `equus/docs/features/workplaceRelationship.md` — User ↔ role profile workplace relationship (no separate business account)
- [`equus/docs/product/benchMarket/webapps.md`](benchMarket/webapps.md#12-equinem) — EquineM competitor baseline (stable ERP parity)
...
- `equus/docs/README.md` — documentation index
...
Stable capabilities: [equus/docs/features/stableModule.md](../features/stableModule.md).
...
Each horse is a first-class entity with its own profile. Full operational scope is defined in [equus/docs/features/horseModule.md](../features/horseModule.md) (profile, ownership, discovery, relationships, timeline, health, documents, subscription).
...
**Operations:**
- Full operational scope is defined in [equus/docs/features/stableModule.md](../features/stableModule.md) (activity planning, roster, feed, facilities, finance, team, owner transparency)
- Horse-centric owner features (timeline, discovery, subscription) are defined in [equus/docs/features/horseModule.md](../features/horseModule.md)
...
**Baseline discovery API (shipped):** `POST /api/v1/veterinaries`, discovery PATCH, and public card GET — see [equus/docs/engineering/entities/veterinaries.md](../engineering/entities/veterinaries.md). Full clinical module (treatment records, scheduling, attachments) remains in §10.3.
...
**Baseline API (shipped):** `POST /api/v1/transports`, `PATCH /api/v1/transports/:id/discovery`, `GET /api/v1/transports/:id` — see [equus/docs/engineering/entities/transports.md](../engineering/entities/transports.md).
...
- **Two link types:** horse `Relationship` (horse ↔ provider) and `WorkplaceRelationship` (User ↔ stable profile collaboration). See [equus/docs/features/workplaceRelationship.md](../features/workplaceRelationship.md)
...
- Technical stack → `equus/docs/engineering/stack.md` (Next.js REST API, MongoDB, Auth.js, React Native, Cloudinary)
...
**Full feature list:** [equus/docs/features/stableModule.md](../features/stableModule.md) — EquineM parity (activity planning, roster, feed, facilities, finance, team, reports) plus ecosystem differentiators (discovery, transparency, horse-scoped reviews, owner dashboard).
...
- **Collaborators:** Users invited to collaborate at a stable profile via `WorkplaceRelationship`; hierarchy on the link; `Stable.collaborators[]` indexes active collaborations; **multi-stable** allowed — see [equus/docs/features/workplaceRelationship.md](../features/workplaceRelationship.md)
...
Primary purpose and feature checklist: [equus/docs/features/horseModule.md](../features/horseModule.md).
...
6. **Collaborators are Users** — profile owner invites a User to collaborate at a host **role profile** (e.g. Stable) via `WorkplaceRelationship`; hierarchy on the link — see [equus/docs/features/workplaceRelationship.md](../features/workplaceRelationship.md)
...
Realtime delivery: see `equus/docs/engineering/stack.md` §9.3 (REST first, Socket.io in Equus when live chat requires it).
...
**Public production launch** requires fully implemented modules: **User**, **Horse**, **Veterinary**, and **Stable** (see `mvpScope.md` — Production launch requirements). Stable scope is defined in [equus/docs/features/stableModule.md](../features/stableModule.md).
...
Three kinds (see [equus/docs/features/ownershipTransfer.md](../features/ownershipTransfer.md)):
...
Technical implementation: see `equus/docs/engineering/stack.md` and `productFlows.md`.
...
Reference competitor baseline: [`equus/docs/product/benchMarket/webapps.md`](benchMarket/webapps.md#12-equinem) (EquineM).  
Stable capability target: [equus/docs/features/stableModule.md](../features/stableModule.md).
...
Destination: **match EquineM stable operations** (see `equus/docs/features/stableModule.md` parity sections) and **exceed** with ecosystem features EquineM does not center (Section 20.5).

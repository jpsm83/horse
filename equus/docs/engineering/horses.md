...
**Status:** **drift** (planning/document **writes** still on horse; no waiting-transfer)  
...
| `POST` | `/api/v1/horses` | Create (`mainOwnerUserId` = session). Guarded by **owner-tier** billing — do not extend ([`billing.md`](billing.md)) |
...
| `GET`/`POST` | `/api/v1/horses/:id/planning` | List / **create** planning events |
...
- **Writes** for care, invoices, feed, whiteboard, roster: **stable (entity) APIs** — mark `POST …/planning` (and any other ops creates) **stop using / replace**. Owner “reply” on an event = [`chat.md`](chat.md).  

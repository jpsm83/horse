...
Related docs:
- [equus/docs/README.md](../README.md) — documentation index
- [equus/docs/product/businessPlan.md](../product/businessPlan.md) — product vision and domain rules
- [equus/docs/features/userModule.md](../features/userModule.md) — one login, optional roles, layered discovery
- [equus/docs/product/mvpScope.md](../product/mvpScope.md) — what to build first
- [equus/docs/product/productFlows.md](../product/productFlows.md) — user journeys
...
Live chat is open between users (see `equus/docs/product/businessPlan.md`). Realtime delivery can stay **inside the Next.js Node process** — no separate websocket microservice for MVP.
...
Items we may add later. Nothing here is required for Phase 1A/1B wedge pilots. **Public production** requires User, Horse, Veterinary, and Stable modules per `equus/docs/product/mvpScope.md`.
...
1. **Shipping to production** — User, Horse, Veterinary, and Stable modules complete (`equus/docs/product/mvpScope.md`)
...
|-------------|-------------|
| `useEffect` + `useState` for async data | `useQuery()` |

/**
 * TransportCreateContent — create-transport form (`/transport/new`).
 *
 * React Hook Form + Zod. Submits to `POST /api/v1/transports` and redirects to
 * the new transport hub. Includes identity, contact, and address fields.
 */
...
import { useCreateTransport } from "@/hooks/queries/useTransport.ts";
import { useRouter } from "@/i18n/navigation.ts";

const createTransportFormSchema = z.object({
...
type CreateTransportFormValues = z.infer<typeof createTransportFormSchema>;

const emptyValues: CreateTransportFormValues = {
  companyName: "",
  description: "",
  email: "",
  phoneNumber: "",
...
export function TransportCreateContent() {
  const router = useRouter();
  const t = useTranslations("transport.create");
  const tCommon = useTranslations("common");
  const [apiError, setApiError] = useState<string | null>(null);
  const createTransport = useCreateTransport();

  const schema = useMemo(() => createTransportFormSchema, []);
  const form = useForm<CreateTransportFormValues>({
  ...
  async function onSubmit(values: CreateTransportFormValues) {
    setApiError(null);
    try {
      const { transport } = await createTransport.mutateAsync({
      ...
      router.push(`/transport/${transport._id}`);
      router.refresh();
    } catch (err) {

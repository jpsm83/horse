...
import { sendStaffInviteEmail } from "@/lib/email/sendStaffInviteEmail.ts";
...
import {
  ...
  createTestStable,
  createTestTransport,
} from "@/models/__tests__/helpers/businessRoleFixtures.ts";
...
describe("workplaceRelationshipService", () => {
  ...
  it("rolls back invite when staff email send fails", async () => {
    vi.mocked(sendStaffInviteEmail).mockRejectedValueOnce(new Error("SMTP down"));

    const owner = await createOwner("email-fail-owner@example.com");
    const stable = await createTestStable(owner._id);
    ...
    await expect(
      workplaceRelationshipService.inviteCollaborator(
        ...
        "stable",
        String(stable._id),
        inviteInput("email-fail-staff@example.com", "staff"),
      ),
    ).rejects.toMatchObject({ statusCode: 500 });

    const count = await WorkplaceRelationship.countDocuments({

...
describe("discoverService", () => {
  it("returns public stables matching search query", async () => {
    ...
    await createTestStable(String(stableOwner._id), { tradeName: "Valley Stable" });
    ...
    expect(results.some((entry) => entry.label === "Valley Stable")).toBe(false);
  });
  ...
  it("allows host scope for service types", async () => {
    ...
    const { createTestGroom } = await import("@/models/__tests__/helpers/businessRoleFixtures.ts");
  ...
  it("excludes profiles operated by deactivated users", async () => {
    ...
    await createTestStable(String(stableOwner._id), { tradeName: "Ghost Stable" });

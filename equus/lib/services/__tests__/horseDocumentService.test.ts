...
describe("createHorseDocument", () => {
  it("creates a document for the horse owner team", async () => {
    await connectDb();
    const userObjId = new mongoose.Types.ObjectId();
    const horse = await Horse.create({
      mainOwnerUserId: userObjId,
  ...
  it("rejects create from a non-owner", async () => {
    ...
    await expect(
      createHorseDocument(strangerId.toHexString(), horse._id.toHexString(), {
        documentType: "insurance",
        title: "Blocked",
        fileUrl: "https://example.com/f.pdf",
        fileName: "f.pdf",
...
describe("getHorseDocumentDownloadMeta", () => {
  ...
  it("throws 403 when user cannot access the documents tab", async () => {
    const strangerId = new mongoose.Types.ObjectId().toHexString();

    await expect(getHorseDocumentDownloadMeta(strangerId, horseId, docId)).rejects.toMatchObject({
      statusCode: 403,
    });
  });

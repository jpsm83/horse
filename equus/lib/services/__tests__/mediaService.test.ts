...
import { deleteMedia, createMedia, listMedia, listHorseHubGallery, updateMediaHubVisibility } from "@/lib/services/mediaService";
...
describe("updateMediaHubVisibility", () => {
  let userId: string;
  let horseId: string;
  let mediaId: string;
  ...
  beforeEach(async () => {
    ...
    const media = await Media.create({
      ...
      uploadedByUserId: userObjId,
      type: "image",
      url: "https://res.cloudinary.com/test/image/upload/v1/vis.jpg",
      isVisibleOnHub: true,
    });
    mediaId = media._id.toHexString();
  });
  ...
  it("updates isVisibleOnHub for the horse owner", async () => {
    const result = await updateMediaHubVisibility(userId, horseId, mediaId, false);
    expect(result.isVisibleOnHub).toBe(false);

    const stored = await Media.findById(mediaId).lean();
    expect(stored?.isVisibleOnHub).toBe(false);
  });

  it("throws 404 when user is not on the ownership team", async () => {
    const strangerId = new mongoose.Types.ObjectId().toHexString();
    await expect(
      updateMediaHubVisibility(strangerId, horseId, mediaId, false),
    ).rejects.toThrow("Horse not found");
  });
  ...
  it("throws 404 when media belongs to a different horse", async () => {
    ...
    await expect(
      updateMediaHubVisibility(userId, otherHorse._id.toHexString(), mediaId, false),
    ).rejects.toThrow("Media not found");
  });
});

describe("createMedia", () => {
  it("creates a media record for the horse owner team", async () => {
    await connectDb();
    const email = `media-create-owner-${Date.now()}@example.com`;
    ...
    const horse = await Horse.create({
      ...
      name: "Create Media Horse",
      breed: "Arabian",
      sex: "Mare",
    });
    const horseId = horse._id.toHexString();

    const result = await createMedia(ownerId, horseId, {
      type: "image",
  ...
  it("rejects create from a non-owner", async () => {
    await connectDb();
    const ownerEmail = `media-create-owner2-${Date.now()}@example.com`;
    const strangerEmail = `media-create-stranger-${Date.now()}@example.com`;
    const owner = await User.create({
      personalDetails: { email: ownerEmail, password: "hash" },
      authProvider: "credentials",
    });
    const stranger = await User.create({
      personalDetails: { email: strangerEmail, password: "hash" },
      authProvider: "credentials",
    });
    const horse = await Horse.create({
      mainOwnerUserId: owner._id,
      createdByUserId: owner._id,
      name: "Create Forbid Horse",
      breed: "Arabian",
      sex: "Mare",
    });

    await expect(
      createMedia(stranger._id.toHexString(), horse._id.toHexString(), {
        type: "image",

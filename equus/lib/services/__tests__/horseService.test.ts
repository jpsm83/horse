...
describe("horseService", () => {
  it("creates horse with main owner and creator ids", async () => {
    ...
    expect((horse.registration as { isActive?: boolean })?.isActive).toBe(true);
  });

  it("allows free-tier users to create more than one horse (no horse-count cap)", async () => {
    const owner = await createUser("horse-free-tier-multi@example.com");
    ...
    const second = await horseService.createHorse(String(owner._id), {
      name: "Second",
      breed: "Arabian",
      sex: "Mare",
      countryOfBirth: "US",
    ...
    const listed = await horseService.listHorses(String(owner._id), { mine: true });

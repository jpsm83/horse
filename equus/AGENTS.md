...
* **Horse entity tables** — use `components/table` (`DataTable` + `TableUserAvatarCell` / `TableRowAction` / `TableIconAction` / `initialsFromLabel`). Admin History is the visual SoT; do not re-copy avatar/action markup. See [`documentation/page-flow-blueprint.md`](documentation/page-flow-blueprint.md) §6 (Horse entity tables).
